...
public class CardConvert {
    ...
    public CardDTO toObject(JsonNode card) {
        return new CardDTO(
                ...
                card.get("level") != null? Integer.parseInt(card.get("level").toString()) : 0,
                card.get("desc").toString().replaceAll("\"", ""),
                card.get("atk") != null ? new BigDecimal(card.get("atk").toString()) : null,
                card.get("def") != null ? new BigDecimal(card.get("def").toString()) : null,
                card.get("race").toString() != null ? card.get("race").toString().replaceAll("\"", "") : "",
                card.get("attribute") != null ? card.get("attribute").toString().replaceAll("\"", "") : "",
